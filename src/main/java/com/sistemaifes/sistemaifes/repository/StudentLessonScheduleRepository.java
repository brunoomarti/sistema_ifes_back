     ...
     WITH NextLesson AS (
        ...
        JOIN
         ...
         WHERE
            ...
            AND TO_TIMESTAMP(CONCAT(CURRENT_DATE::text, ' ', s.start_time), 'YYYY-MM-DD HH24:MI') > NOW()
            AND CASE 
            ...
            END = a.week_day
         ORDER BY
            TO_TIMESTAMP(CONCAT(CURRENT_DATE::text, ' ', s.start_time), 'YYYY-MM-DD HH24:MI') ASC
         LIMIT 1
        )
        SELECT
             nl.student_name,
